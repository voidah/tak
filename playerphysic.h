...
{
    public:
        PlayerPhysic(const std::string& name, const Vector3f& size, float weightKg, float eyeHeight);
        ~PlayerPhysic();
    ...
    {
        public:
            MotionState(PlayerPhysic* player) : m_player(player)
        {
            //m_pos.setIdentity();
        }
    ...
    {
        public:
            PlayerRigidBody(PlayerPhysic* player, MotionState* motionState) : RigidBody(0)
        {
